...
export default async function HomePage() {
  const data = {
    name: 'Jessé Ávila',
    avatar: 'https://avatars.githubusercontent.com/u/45925514?v=4',
    links: [
      {
          platform_id: 2,
      ...
      },
      {
          ...
          url: "https://www.instagram.com/jesse.avilaa/"
      },
      {
          platform_id: 4,
          platform_name: 'Github',
          url: "https://github.com/JesseAvilaa"
      }
  ],
  }
  ...
  return (
    <div className="flex items-center flex-col mx-auto w-full justify-center mt-16 px-8">
      ...
      {data.links.map((link) => (
        <LinkCard href={link.url} title={link.platform_name} key={link.url} {...link} />
      ))}
      <div className="flex items-center gap-4 mt-8 text-white">
        {data.links.map((social) => (
          <a
            aria-label={`${social.platform_name} link`}
            key={social.url}
            href={social.url}
            target="_blank"
            rel="noopener noreferrer"
